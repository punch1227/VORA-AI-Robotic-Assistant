...
from gateway.intent_parser import (
    ...
    normalize_thai_command, parse_control_intent, looks_like_command
)
...
# Tracks the last non-empty TTS utterance so "พูดอีกครั้ง" can replay it.
_last_tts_text: str = ""
...
# ============================================================
# Task-Oriented Command Router
# ------------------------------------------------------------
# Handles the six deterministic priority intents BEFORE wake-word
# and LLM routing. Logging here is the authoritative record for
# STT/intent evaluation experiments.
# ============================================================

OUT_OF_SCOPE_TH = "ขออภัยครับ ตอนนี้ผมรองรับคำสั่งควบคุมหุ่นยนต์และการค้นหาวัตถุเป็นหลัก"
CLARIFY_TH = "ขออภัยครับ ผมฟังไม่ชัด ช่วยพูดคำสั่งอีกครั้งได้ไหมครับ"


def _log_utterance(raw: str, normalized: str, intent: str, method: str,
                   routed: str, response_type: str):
    """Structured evaluation log for every processed utterance."""
    logger.info(
        "[INTENT_EVAL] raw=%r norm=%r intent=%s method=%s routed=%s resp=%s",
        raw, normalized, intent, method, routed, response_type,
    )


async def handle_control_intent(intent_name: str, raw: str, normalized: str) -> bool:
    """Dispatch one of the six priority control intents. Returns True if handled."""
    global _last_tts_text

    if intent_name == "stop":
        _log_utterance(raw, normalized, "stop", "deterministic", "motion.stop", "tts_short")
        if _search_active:
            await cancel_search()
        if not MOCK_ROBOT:
            try:
                await motion.exec_motion({"type": "stop"}, obstacle_checker=_lidar_obstacle_check)
            except Exception as e:
                logger.warning(f"stop dispatch error: {e}")
        await speak_tts("หยุดแล้วครับ")
        return True

    if intent_name == "cancel":
        _log_utterance(raw, normalized, "cancel", "deterministic", "cancel_search+stop", "tts_short")
        if _search_active:
            await cancel_search()
        if not MOCK_ROBOT:
            try:
                await motion.exec_motion({"type": "stop"}, obstacle_checker=_lidar_obstacle_check)
                if _nav2_client and getattr(_nav2_client, "connected", False):
                    await _nav2_client.cancel_navigation()
            except Exception as e:
                logger.warning(f"cancel dispatch error: {e}")
        await speak_tts("ยกเลิกคำสั่งแล้วครับ")
        return True

    if intent_name == "repeat_last_response":
        reply = _last_tts_text or "ยังไม่มีคำพูดก่อนหน้านี้ครับ"
        _log_utterance(raw, normalized, "repeat_last_response", "deterministic", "tts_replay", "tts_short")
        # Bypass the _last_tts_text update so repeat-of-repeat still replays the
        # original utterance.
        prev = _last_tts_text
        await speak_tts(reply)
        _last_tts_text = prev or reply
        return True

    if intent_name == "report_status":
        parts = []
        if _search_active and _current_search_target:
            parts.append(f"กำลังค้นหา {_current_search_target}")
        else:
            parts.append("ระบบพร้อมทำงาน")
        try:
            if _robot_pose and isinstance(_robot_pose, dict):
                x = _robot_pose.get("x"); y = _robot_pose.get("y")
                if x is not None and y is not None:
                    parts.append(f"ตำแหน่งอยู่ที่พิกัด x {x:.2f} y {y:.2f}")
        except Exception:
            pass
        reply = " ".join(parts) + "ครับ"
        _log_utterance(raw, normalized, "report_status", "deterministic", "status_summary", "tts_short")
        await speak_tts(reply)
        return True

    if intent_name == "where_am_i":
        reply = "ตอนนี้อยู่ในห้องแล็บครับ"
        try:
            zone_label = None
            try:
                from gateway import semantic_map as _sm
                if _robot_pose and isinstance(_robot_pose, dict):
                    z = _sm.get_zone_at(_robot_pose.get("x", 0.0), _robot_pose.get("y", 0.0))
                    if z is not None:
                        zone_label = getattr(z, "label_th", None) or getattr(z, "id", None)
            except Exception:
                pass
            if zone_label:
                reply = f"ตอนนี้อยู่ที่ {zone_label} ครับ"
            elif _robot_pose and isinstance(_robot_pose, dict):
                x = _robot_pose.get("x"); y = _robot_pose.get("y")
                if x is not None and y is not None:
                    reply = f"ตอนนี้อยู่ที่พิกัด x {x:.2f} y {y:.2f} ครับ"
        except Exception:
            pass
        _log_utterance(raw, normalized, "where_am_i", "deterministic", "pose_lookup", "tts_short")
        await speak_tts(reply)
        return True

    if intent_name == "return_home":
        _log_utterance(raw, normalized, "return_home", "deterministic", "planner.home", "tts_short")
        await speak_tts("กำลังกลับจุดเริ่มต้นครับ")
        try:
            if not MOCK_ROBOT:
                # Reuse existing planner path — do NOT touch nav stack internals.
                await try_plan_and_execute("กลับแท่นชาร์จ", "th")
        except Exception as e:
            logger.warning(f"return_home dispatch error: {e}")
        return True

    return False


async def _task_oriented_fallback(raw: str, normalized: str, command_like: bool) -> None:
    """Safe fallback for unmatched utterances in normal runtime mode.

    - command-like shape → ask to repeat briefly.
    - anything else (general-chat / knowledge) → short out-of-scope reply.
    Never leaks architecture or system details.
    """
    if command_like:
        _log_utterance(raw, normalized, "unknown", "fallback", "clarify", "tts_short")
        await speak_tts(CLARIFY_TH)
    else:
        _log_utterance(raw, normalized, "out_of_scope", "fallback", "refuse", "tts_short")
        await speak_tts(OUT_OF_SCOPE_TH)
...
# ── Session isolation ─────────────────────────────────────────────────
# _search_session_id is a monotonic counter bumped at each visual_search start
# AND at each cancel_search(). Every running task snapshots its own sid at start
# and uses _session_owner() to verify it still owns the active session before
# mutating any shared state. A stale task whose sid != current id must exit
# without touching globals (prevents cross-session state leaks).
_search_session_id: int = 0
# Handle to the currently-running visual_search task so a new entry point can
# cancel+await the prior task cleanly instead of racing with an untracked
# asyncio.sleep(1). Reset to None when cleared by its owning session.
_active_search_task: Optional[asyncio.Task] = None


def _abs_to_delta_angles(abs_angles: List[float]) -> List[float]:
    """Convert a list of absolute scan headings (degrees, relative to entry
    heading at index 0) into cumulative rotation deltas suitable for the
    relative-rotation _rotate_deg() primitive.

    Example: [0, 120, -120] → [0, +120, -240]
    After applying each delta in order the robot faces the intended absolute
    heading at every step — 3 unique positions, full 360° coverage."""
    deltas: List[float] = []
    prev = 0.0
    for a in abs_angles:
        deltas.append(a - prev)
        prev = a
    return deltas


def _lidar_ranked_scan_headings(oa, k: int = 3) -> List[float]:
    """Return up to k absolute scan headings (degrees, relative to robot's
    current facing, + = left) ranked by LiDAR forward-clearance openness.

    Candidates: [0, ±45, ±90, ±110] — all within the YDLidar G2 ±115° arc.
    Dead zones (no rays at that angle) are skipped.
    Falls back to [0, +90, −90] when LiDAR has no data or all candidates
    are dead-zones.

    Scoring per candidate:
        score = fwd_s * 0.55 + side_s * 0.20 − wall_pen − angle_pen * 0.10
    where:
        fwd_s     = min(fwd_clearance, 2.5m) / 2.5   (normalised openness)
        side_s    = avg of finite side clearances (normalised)
        wall_pen  = 0.35 if fwd_clearance < 0.5m (wall-facing → deprioritise)
        angle_pen = abs(heading) / 180            (small bias toward forward)
    """
    _FALLBACK = [0.0, 90.0, -90.0]
    _CAP_M = 2.5     # beyond this distance, extra space adds no scan value
    _WALL_M = 0.5    # heading is wall-facing when nearest obstacle < this
    _ARC_LIM = 110.0  # 5° inside ±115° YDLidar G2 physical scan arc

    candidates = [0.0, 45.0, -45.0, 90.0, -90.0, 110.0, -110.0]

    if oa is None or not getattr(oa, "_last_scan_ranges", None):
        logger.info("[SCAN_PLAN] LiDAR no-data → fallback headings [0,+90,−90]")
        return _FALLBACK
    ...
    for h in candidates:
        if abs(h) > _ARC_LIM:
            continue
        fwd, left, right = oa._directional_clearance(h, window_deg=12.0)
        if not math.isfinite(fwd):
            logger.debug(f"[SCAN_PLAN] {h:+.0f}°: dead-zone skip")
            continue
        fwd_s = min(fwd, _CAP_M) / _CAP_M
        valid_sides = [v for v in (left, right) if math.isfinite(v)]
        side_s = (
            min(sum(valid_sides) / len(valid_sides), _CAP_M) / _CAP_M
            if valid_sides else 0.5
        )
        wall_pen = 0.35 if fwd < _WALL_M else 0.0
        angle_pen = abs(h) / 180.0
        score = fwd_s * 0.55 + side_s * 0.20 - wall_pen - angle_pen * 0.10
        scored.append((h, score))
        left_txt = f"{left:.2f}" if math.isfinite(left) else "∞"
        right_txt = f"{right:.2f}" if math.isfinite(right) else "∞"

        logger.info(
            f"[SCAN_PLAN] {h:+.0f}°: fwd={fwd:.2f}m "
            f"sides=({left_txt},{right_txt}) "
            f"wall={'Y' if wall_pen else 'N'} score={score:.3f}"
        )

    if not scored:
        logger.info("[SCAN_PLAN] all candidates dead-zone → fallback [0,+90,−90]")
        return _FALLBACK

    scored.sort(key=lambda x: x[1], reverse=True)
    chosen = [h for h, _ in scored[:k]]
    logger.info(
        f"[SCAN_PLAN] LiDAR-ranked plan (top {len(chosen)} of {len(scored)}): "
        f"{[f'{h:+.0f}°({s:.3f})' for h, s in scored[:k]]}"
    )
    return chosen


def _heading_covered(h: float, visited: set, tol: float = 20.0) -> bool:
    """True if heading h is within tol° of any already-visited heading.

    Prevents re-scanning views that are effectively identical (within ±20°)
    without requiring exact heading equality. Uses modular angular distance.
    """
    for v in visited:
        if abs((h - v + 180) % 360 - 180) <= tol:
            return True
    return False
...
async def cancel_search():
    ...
    Also bumps _search_session_id atomically so any in-flight task (mid-VLM,
    mid-rotate, mid-nudge) will observe its snapshot sid != current id on its
    next _session_owner() check and self-terminate without mutating globals —
    even if a newer search immediately resets _search_cancel."""
    global _search_cancel, _search_session_id
    if _search_active:
        _search_cancel = True
        _search_session_id += 1
        _search_cancel_event.set()
        logger.info(f"🛑 [SEARCH] cancel requested — session bumped to {_search_session_id}")


async def _start_visual_search_task(coro_factory) -> asyncio.Task:
    """Atomic handoff: cancel any prior visual_search, await its finally (bounded),
    then launch a new one. Stores the handle in _active_search_task so the NEXT
    entry point can do the same.

    ``coro_factory`` is a zero-arg callable returning a fresh coroutine to run
    (defers coroutine creation until after we've awaited the prior task).
    Returns the new Task handle."""
    global _active_search_task
    prior = _active_search_task
    if prior is not None and not prior.done():
        await cancel_search()
        try:
            await asyncio.wait_for(asyncio.shield(prior), timeout=5.0)
            logger.info("[SEARCH] prior task finished — starting new search")
        except asyncio.TimeoutError:
            logger.warning(
                "[SEARCH] prior task still running after 5s — starting new search "
                "anyway (session guard will neutralize stale work)"
            )
        except Exception as _e:
            logger.info(f"[SEARCH] prior task ended with {type(_e).__name__} — starting new")
    _active_search_task = asyncio.create_task(coro_factory())
    return _active_search_task
...
async def visual_search(
    ...
    """
    global _search_active, _search_cancel, _current_zone, _current_search_target, _search_session_id
    # ── Fresh session id ──────────────────────────────────────────────
    # Bump BEFORE enabling _search_active so cancel_search() can't bump
    # underneath us and flip the id twice. Snapshot to _my_sid — all later
    # mutations guarded by _session_owner() use this snapshot.
    _search_session_id += 1
    _my_sid: int = _search_session_id
    _search_active = True
    _search_cancel = False
    _search_cancel_event.clear()
    _current_search_target = target_object

    def _session_owner() -> bool:
        """True iff this task still owns the active search session.
        A newer visual_search() or cancel_search() call bumps the global id;
        once that happens this task must exit without mutating shared state."""
        return _my_sid == _search_session_id
    ...
    logger.info(f"═" * 50)
    logger.info(f"🔍 VISUAL SEARCH START (sid={_my_sid}): '{target_object}'")
    ...
    # Terminal-reason tracker: every return path sets this before returning so the
    # finally block can log WHY the session ended. "transition_*" values MUST NOT
    # appear at finally — they indicate a phase handoff (nav→local) that should
    # never be the last state. If we see a transition value in finally, that is a
    # true handoff bug.
    _end_reason: str = "unspecified"
    _local_search_started: bool = False  # True once LOCAL SEARCH MODE has begun
    # Nav-success observer trackers. These flip the MOMENT a successful nav
    # result is observed (directly from _navigate_with_watchdog or via the
    # reconciliation salvage). They remain True even if a CancelledError /
    # BaseException later propagates — so the finally block can DEFINITIVELY
    # detect the "nav succeeded but LOCAL_SEARCH never ran" bug regardless of
    # which exception type ate the control flow.
    _nav_attempted: bool = False
    _nav_success_observed: bool = False
    _nav_goal_xy: tuple = (0.0, 0.0)
    _nav_zone_id_at_success: Optional[str] = None
    ...
    async def _do_vlm_and_check(phase_label: str, force: bool = False, broadcast_extra: dict = None) -> bool:
        ...
        if found:
            # ── ZONE LOCK GUARD ──
            # When user named a target zone, FOUND is only terminal if the robot
            # is currently inside that zone — regardless of arrived_zone.
            # arrived_zone=True does NOT unlock this: the robot may have drifted
            # out during expansion, or the sweep may be visiting a different zone.
            # _early_tgt_zone is used (not _explicit_zone_id) because it is
            # assigned before _do_vlm_and_check is first called (Phase 0).
            if _early_tgt_zone:
                try:
                ...
                if _cz_now_id == _early_tgt_zone:
                    ...
                    # Fallback: check robot pose against target-zone radius (+0.5m slop)
                    try:
                        _zl_tgt = semantic_map.get_zone(_early_tgt_zone)
                        if _zl_tgt is not None:
                            _zl_px = _robot_pose.get("x", 0.0)
                            _zl_py = _robot_pose.get("y", 0.0)
                            _zl_dx = _zl_px - _zl_tgt.center_x
                            _zl_dy = _zl_py - _zl_tgt.center_y
                            _zl_dist = math.sqrt(_zl_dx * _zl_dx + _zl_dy * _zl_dy)
                            if _zl_dist <= _zl_tgt.radius + 0.5:
                                _zl_in_zone = True
                                logger.info(
                                    f"[ZONE_LOCK] pose within target zone radius "
                                    f"(dist={_zl_dist:.2f}m <= {_zl_tgt.radius + 0.5:.2f}m) → allow FOUND"
                                )
                    except Exception:
                        pass
                if not _zl_in_zone:
                    logger.warning(
                        f"[ZONE_LOCK] explicit target='{_early_tgt_zone}' "
                        f"current='{_cz_now_id}' → suppress terminal FOUND "
                        f"(incidental sighting outside target zone)"
                    )
                    consecutive_found = 0
                    return False
                if _cz_now_id == _early_tgt_zone:
                    logger.info(
                        f"[ZONE_LOCK] current='{_cz_now_id}' matches target → allow FOUND"
                    )
            consecutive_found += 1
    ...
    try:
        ...
        if not _search_cancel:
            ...
            if (
                ...
                arrived_zone = True
                _end_reason = "transition_same_zone_skip"
                logger.info(
                    f"[SEARCH sid={_my_sid}] HANDOFF same-zone → LOCAL_SEARCH "
                    f"(session NOT terminal; state preserved)"
                )
            ...
            if _zone_match and USE_NAV2 and _nav2_client and not MOCK_ROBOT:
                ...
                if not _nav_available:
                    ...
                    if not await wait_for_localization_ready(timeout=10.0):
                        ...
                        _MAX_ATTEMPTS = 2
                        _nav_attempted = True
                        _nav_goal_xy = (_zone_match.center_x, _zone_match.center_y)
                        for _attempt in range(_MAX_ATTEMPTS):
                            ...
                            try:
                                ...
                                logger.error(
                                    f"[SEARCH sid={_my_sid}] CancelledError inside "
                                    f"_navigate_with_watchdog — checking late arrival before abort "
                                    f"(arrived_zone={arrived_zone})"
                                )
                                # Reconcile exactly like the FAIL path:
                                # if robot is already effectively at the target zone/goal,
                                # salvage this as ARRIVED and continue into LOCAL_SEARCH.
                                _ARRIVAL_RADIUS = 0.35  # tightened: zone-match or AMCL-only
                                _gx, _gy = _zone_match.center_x, _zone_match.center_y
                                _recon_reason = None

                                _rx = _robot_pose.get("x", 0.0)
                                _ry = _robot_pose.get("y", 0.0)
                                _ax, _ay = _amcl_watchdog_xy
                                _ox, _oy = _odom_watchdog_xy
                                ...
                                for _src, _cx, _cy in _candidates:
                                    try:
                                        _pz = semantic_map.get_zone_at(_cx, _cy)
                                        _pz_id = _pz.id if _pz else None
                                    except Exception:
                                        _pz_id = None

                                    _dist = math.hypot(_cx - _gx, _cy - _gy)

                                    if _pz_id == _zone_match.id:
                                        _recon_reason = (
                                            f"zone_match via {_src} "
                                            f"pose=({_cx:.2f},{_cy:.2f}) dist={_dist:.2f}m"
                                        )
                                        break

                                    # Radius fallback: AMCL only — dead-reckoned /
                                    # robot_pose alone are unreliable after a timeout.
                                    if _src == "amcl" and _dist <= _ARRIVAL_RADIUS:
                                        _recon_reason = (
                                            f"within_arrival_radius via {_src} "
                                            f"pose=({_cx:.2f},{_cy:.2f}) dist={_dist:.2f}m "
                                            f"<= {_ARRIVAL_RADIUS}m"
                                        )
                                        break
                                ...
                                if _recon_reason:
                                    logger.warning(
                                        f"[NAV] CANCELLEDERROR SALVAGED_AS_ARRIVED "
                                        f"zone='{_zone_match.id}' — {_recon_reason} "
                                        f"→ continuing into LOCAL SEARCH MODE"
                                    )
                                    _zone_nav_ok = True
                                    _nav_success_observed = True
                                    _nav_zone_id_at_success = _zone_match.id
                                    _end_reason = "transition_nav_success_cancelled_but_arrived"
                                    _search_cancel = False
                                    _search_cancel_event.clear()
                                    break

                                _end_reason = "nav_cancelled_error_in_retry_loop"
                                logger.error(
                                    f"[SEARCH sid={_my_sid}] CancelledError was a TRUE abort "
                                    f"(not arrived) — re-raising"
                                )
                                raise   
                            except BaseException as _e_inner:
                                logger.warning(f"[NAV] go_to_zone error attempt {_attempt+1}: {_e_inner!r}")
                                _zone_nav_ok = False
                            # ── Nav-success observed ────────────────────────
                            # Tag the observer flag + _end_reason HERE, the
                            # moment success is first observed. Prior code
                            # deferred this to L1470, leaving a window where
                            # an uncaught CancelledError could reach `finally`
                            # with reason=unspecified even though nav had in
                            # fact succeeded.
                            if _zone_nav_ok:
                                _nav_success_observed = True
                                _nav_zone_id_at_success = _zone_match.id
                                _end_reason = "transition_nav_success"
                                logger.info(
                                    f"[SEARCH sid={_my_sid}] nav_success_observed=True "
                                    f"zone='{_zone_match.id}' attempt={_attempt+1} "
                                    f"(tagged _end_reason EARLY to survive any later cancel)"
                                )
                                break
                            logger.warning(
                ...
                # Post-nav reconciliation: Nav2 / watchdog can fire
                # ACCEPTANCE_TIMEOUT, ABORTED, or stall-cancel AFTER the robot
                # has physically reached the target. Before declaring failure,
                # (1) allow a short arrival grace for odom/AMCL to settle,
                # (2) re-check zone containment against the freshest pose AND
                #     the watchdog AMCL/odom snapshots,
                # (3) fall back to distance-to-goal within ARRIVAL_RADIUS.
                # Only then is it a true HARD_FAIL.
                if not _zone_nav_ok and _nav_available:
                    _ARRIVAL_RADIUS = 0.35  # tightened: zone-match or AMCL-only
                    _GRACE_S = 2.0          # s — total grace window
                    _POLL_S = 0.2
                    _gx, _gy = _zone_match.center_x, _zone_match.center_y
                    logger.info(
                        f"[NAV] raw nav result: FAIL — starting reconciliation "
                        f"(goal=({_gx:.2f},{_gy:.2f}) zone='{_zone_match.id}')"
                    )
                    _recon_reason: Optional[str] = None
                    _t0 = time.monotonic()
                    while time.monotonic() - _t0 < _GRACE_S:
                        _rx = _robot_pose.get("x", 0.0)
                        _ry = _robot_pose.get("y", 0.0)
                        _ax, _ay = _amcl_watchdog_xy
                        _ox, _oy = _odom_watchdog_xy
                        _candidates = [("robot_pose", _rx, _ry),
                                       ("amcl", _ax, _ay),
                                       ("odom", _ox, _oy)]
                        for _src, _cx, _cy in _candidates:
                            try:
                            ...
                            if _pz_id == _zone_match.id:
                                ...
                                break
                            # Radius fallback: AMCL only — dead-reckoned /
                            # robot_pose alone are unreliable after nav failure.
                            if _src == "amcl" and _dist <= _ARRIVAL_RADIUS:
                                _recon_reason = (
                                    f"within_arrival_radius via {_src} "
                                    f"pose=({_cx:.2f},{_cy:.2f}) dist={_dist:.2f}m "
                    ...
                    if _recon_reason:
                        logger.warning(
                            f"[NAV] RECONCILE SALVAGED_AS_ARRIVED "
                            f"zone='{_zone_match.id}' — {_recon_reason} "
                            f"→ continuing into LOCAL SEARCH MODE"
                        )
                        logger.info(
                            f"[SEARCH sid={_my_sid}] raw_nav_result=FAIL→SALVAGED "
                            f"zone='{_zone_match.id}' "
                            f"→ TRANSITION_TO_LOCAL_SEARCH (session NOT terminal; "
                            f"state preserved)"
                        )
                        _zone_nav_ok = True
                        # Nav-success observed via reconciliation. Tag EARLY
                        # so finally can never see reason=unspecified when
                        # the robot did in fact arrive.
                        _nav_success_observed = True
                        _nav_zone_id_at_success = _zone_match.id
                        if not _end_reason.startswith("transition_"):
                            _end_reason = "transition_nav_success_salvaged"
                    else:
                        _rx = _robot_pose.get("x", 0.0)
                        _ry = _robot_pose.get("y", 0.0)
                        _ax, _ay = _amcl_watchdog_xy
                        _ox, _oy = _odom_watchdog_xy
                        _dist_final = math.hypot(_rx - _gx, _ry - _gy)
                        logger.warning(
                            f"[NAV] RECONCILE HARD_FAIL zone='{_zone_match.id}' "
                            f"robot=({_rx:.2f},{_ry:.2f}) "
                            f"amcl=({_ax:.2f},{_ay:.2f}) "
                            f"odom=({_ox:.2f},{_oy:.2f}) "
                            f"dist_to_goal={_dist_final:.2f}m > {_ARRIVAL_RADIUS}m"
                        )
                ...
                if _zone_nav_ok:
                    ...
                    logger.info(
                        f"[SEARCH sid={_my_sid}] raw_nav_result=OK(SUCCEEDED) "
                        f"zone='{_zone_match.id}' "
                        f"→ TRANSITION_TO_LOCAL_SEARCH (session NOT terminal; "
                        f"state preserved; NOT clearing _search_active)"
                    )
                    logger.info(
                    ...
                    # Clear any mid-transit cancel flag: a cancel fired DURING nav
                    # (e.g. stall-triggered cancel_navigation + user timing) must
                    # not leak into the local-search phase. The user asked for an
                    # object search WITH a zone — nav arrival is a phase transition,
                    # not a terminal event. Local scan will re-check cancel itself.
                    if _search_cancel:
                        logger.info(
                            f"[SEARCH sid={_my_sid}] mid-transit cancel flag seen at "
                            f"nav-success — clearing for LOCAL_SEARCH handoff "
                            f"(object-search-with-zone: nav success ≠ session end)"
                        )
                        _search_cancel = False
                        _search_cancel_event.clear()
                elif _nav_available:
        ...
        # ── HARD GUARD ──────────────────────────────────────────────────
        # If _nav_success_observed is True, this cancel check MUST NOT exit
        # the session. Nav success for object-search-with-zone is a phase
        # transition, never a terminal event. Force a drop-through regardless
        # of any stale cancel flag or any `arrived_zone=False` residual state
        # (defence against assignment-skipped cases).
        if _nav_success_observed and not arrived_zone:
            logger.error(
                f"[SEARCH sid={_my_sid}] INVARIANT_REPAIR: _nav_success_observed=True "
                f"but arrived_zone=False at post-Phase-0.4 gate — forcing arrived_zone=True "
                f"(goal_xy={_nav_goal_xy} zone='{_nav_zone_id_at_success}')"
            )
            arrived_zone = True
            if _nav_zone_id_at_success:
                _current_zone_id = _nav_zone_id_at_success
                _current_zone = _nav_zone_id_at_success
        if _search_cancel and not arrived_zone:
            _end_reason = "terminal_cancel_post_phase04_no_arrival"
            logger.info(
                f"[SEARCH sid={_my_sid}] cancel at post-Phase-0.4 "
                f"(nav_attempted={_nav_attempted} nav_success_observed={_nav_success_observed} "
                f"arrived_zone={arrived_zone})"
            )
            await _search_cancelled(_display)
            return
            ...
            if _plan.approach_zone_id:
                ...
                if _try_fallbacks:
                    ...
                    for _fzid, _fx, _fy in _fallbacks:
                        ...
                        # local_scan_angles() returns ABSOLUTE headings (e.g. [0, 120, -120])
                        # relative to entry heading — convert to cumulative deltas so each
                        # _rotate_deg() lands on the intended absolute heading (no revisit).
                        _fb_abs = SearchPlanner.local_scan_angles()
                        _fb_deltas = _abs_to_delta_angles(_fb_abs)
                        for _fsi, _fsa_delta in enumerate(_fb_deltas):
                            if _search_cancel or not _session_owner():
                                if _session_owner():
                                    await _search_cancelled(_display)
                                return
                            if _fsi > 0:
                                await _rotate_deg(_fsa_delta)
                            if await _do_vlm_and_check(
                ...
                logger.info(
                    ...
                    "starting deterministic ordered zone sweep"
                )

        # ── Ordered zone sweep ────────────────────────────────────────────
        # For zone-unspecified queries: visit entrance → bedroom → bathroom
        # → living_room in fixed order instead of the freeform agent loop.
        if not arrived_zone and not _explicit_zone_id and not _search_cancel:
            _SWEEP_ORDER = ["entrance", "bedroom", "toilet", "living room"]
            _SWEEP_ZONE_NOT_FOUND_TTS: Dict[str, str] = {
                "entrance": "ห้องหน้าประตูยังไม่เจอครับ กำลังไปห้องนอนต่อ",
                "bedroom":  "ห้องนอนยังไม่เจอครับ กำลังไปห้องน้ำต่อ",
                "toilet":   "ห้องน้ำยังไม่เจอครับ กำลังไปห้องนั่งเล่นต่อ",
            }
            _SW_COV_MIN = 6
            _SW_DEG_MIN = 300.0
            _SW_MAX_TURN = 100.0
            _SW_SUPP = [0.0, 45.0, -45.0, 90.0, -90.0, 135.0, -135.0, 180.0]
            logger.info(f"[SWEEP] starting ordered zone sweep for '{normalized_target}': {_SWEEP_ORDER}")
            for _sw_zid in _SWEEP_ORDER:
                if _search_cancel or not _session_owner():
                    if _session_owner():
                        await _search_cancelled(_display)
                    return
                _sw_zone = semantic_map.get_zone(_sw_zid)
                if _sw_zone is None:
                    logger.warning(f"[SWEEP] zone '{_sw_zid}' not in semantic_map — skipping")
                    continue
                _sw_label = _sw_zone.label_th or _sw_zid
                logger.info(f"[SWEEP] → zone '{_sw_zid}' ({_sw_label})")
                speak_tts_bg(f"กำลังไป{_sw_label}ครับ")
                await manager.broadcast(json.dumps({
                    "type": "search_status",
                    "status": "navigating_to_zone",
                    "target": _display,
                    "zone": _sw_zid,
                    "zone_label": _sw_label,
                    "goal_x": _sw_zone.center_x,
                    "goal_y": _sw_zone.center_y,
                }))
                _sw_nav_ok = MOCK_ROBOT
                if USE_NAV2 and _nav2_client and not MOCK_ROBOT:
                    try:
                        _sw_result = await _nav2_client.navigate_to_pose(
                            x=_sw_zone.center_x, y=_sw_zone.center_y,
                            theta=_robot_pose.get("theta", 0.0),
                            timeout=60.0,
                        )
                        _sw_nav_ok = _sw_result.get("success", False)
                        logger.info(f"[SWEEP] nav2 '{_sw_zid}': {'OK' if _sw_nav_ok else 'FAIL'}")
                    except Exception as _sw_e:
                        logger.warning(f"[SWEEP] nav2 error '{_sw_zid}': {_sw_e}")
                if not _sw_nav_ok:
                    logger.info(f"[SWEEP] nav failed '{_sw_zid}' — skipping to next zone")
                    _sw_nf_tts = _SWEEP_ZONE_NOT_FOUND_TTS.get(_sw_zid)
                    if _sw_nf_tts:
                        speak_tts_bg(_sw_nf_tts)
                    continue
                if _search_cancel or not _session_owner():
                    if _session_owner():
                        await _search_cancelled(_display)
                ...
                logger.info(f"[SWEEP] arrived '{_sw_zid}' — Phase A scan")
                _sw_angles = _lidar_ranked_scan_headings(_obstacle_avoidance)
                _sw_visited: set = set()
                _sw_facing = 0.0
                _sw_remaining = list(_sw_angles)
                _sw_step = 0
                while _sw_remaining:
                    _sw_tgt = min(_sw_remaining, key=lambda h: abs((h - _sw_facing + 180) % 360 - 180))
                    _sw_dlt = (_sw_tgt - _sw_facing + 180) % 360 - 180
                    _sw_remaining.remove(_sw_tgt)
                    if _heading_covered(_sw_tgt, _sw_visited):
                        continue
                    if abs(_sw_dlt) > _SW_MAX_TURN:
                        break
                    if _sw_step > 0:
                        if _search_cancel or not _session_owner():
                            if _session_owner(): await _search_cancelled(_display)
                            else: logger.info(f"[SWEEP] stale sid={_my_sid}")
                            return
                        await _rotate_deg(_sw_dlt)
                    _sw_facing = _sw_tgt
                    _sw_visited.add(_sw_tgt)
                    _sw_step += 1
                    logger.info(f"[SWEEP][A] {_sw_zid} heading={_sw_tgt:+.0f}° unique={len(_sw_visited)}")
                    if await _do_vlm_and_check(f"sweep_{_sw_zid}_A{_sw_step}", force=True):
                        return  # found
                _sw_phA = len(_sw_visited)
                if _sw_phA < _SW_COV_MIN and _sw_phA * 45.0 < _SW_DEG_MIN:
                    _sw_supp_rem = [h for h in _SW_SUPP if not _heading_covered(h, _sw_visited)]
                    logger.info(f"[SWEEP][B] {_sw_zid} pool={_sw_supp_rem}")
                    while _sw_supp_rem:
                        _sw_sa = min(_sw_supp_rem, key=lambda h: abs((h - _sw_facing + 180) % 360 - 180))
                        _sw_sd = (_sw_sa - _sw_facing + 180) % 360 - 180
                        _sw_supp_rem.remove(_sw_sa)
                        if _heading_covered(_sw_sa, _sw_visited):
                            continue
                        _sw_cov = len(_sw_visited)
                        if abs(_sw_sd) > _SW_MAX_TURN:
                            if _sw_cov >= _SW_COV_MIN or _sw_cov * 45.0 >= _SW_DEG_MIN:
                                break
                            continue
                        if _search_cancel or not _session_owner():
                            if _session_owner(): await _search_cancelled(_display)
                            ...
                            return
                        await _rotate_deg(_sw_sd)
                        _sw_facing = _sw_sa
                        _sw_visited.add(_sw_sa)
                        _sw_step += 1
                        _sw_cov = len(_sw_visited)
                        logger.info(f"[SWEEP][B] {_sw_zid} heading={_sw_sa:+.0f}° unique={_sw_cov}")
                        if await _do_vlm_and_check(f"sweep_{_sw_zid}_B{_sw_step}", force=True):
                            return  # found
                        if _sw_cov >= _SW_COV_MIN or _sw_cov * 45.0 >= _SW_DEG_MIN:
                            break
                _sw_nf_tts = _SWEEP_ZONE_NOT_FOUND_TTS.get(_sw_zid)
                if _sw_nf_tts:
                    speak_tts_bg(_sw_nf_tts)
                logger.info(f"[SWEEP] '{_sw_zid}' not found — {'continuing' if _sw_nf_tts else 'sweep complete'}")
            speak_tts_bg("ค้นหาครบทุกห้องแล้วแต่ยังไม่พบครับ")
            logger.info(f"[SWEEP] all zones exhausted — '{normalized_target}' not found")
            _end_reason = "terminal_ordered_sweep_not_found"
            await _search_not_found(_display, total_checks)
            return  # never falls to agent loop

        # Second hard guard — same invariant as above, before the explicit
        # zone stop. Nav success MUST imply arrived_zone=True.
        if _nav_success_observed and not arrived_zone:
            logger.error(
                f"[SEARCH sid={_my_sid}] INVARIANT_REPAIR(2): _nav_success_observed=True "
                f"but arrived_zone=False before explicit-zone gate — forcing arrived_zone=True"
            )
            arrived_zone = True
            if _nav_zone_id_at_success:
                _current_zone_id = _nav_zone_id_at_success
                _current_zone = _nav_zone_id_at_success
        if _search_cancel and not arrived_zone:
            _end_reason = "terminal_cancel_pre_local_search"
            await _search_cancelled(_display)
            return
        ...
        if _explicit_zone_id and not arrived_zone:
            # Final safety: if nav was attempted AND succeeded but arrived_zone
            # slipped, do NOT report unreachable. Log loudly.
            if _nav_success_observed:
                logger.error(
                    f"[SEARCH sid={_my_sid}] INVARIANT_VIOLATION: nav_success_observed=True "
                    f"but about to report '{_explicit_zone_id}' unreachable — overriding to "
                    f"LOCAL_SEARCH handoff"
                )
                arrived_zone = True
                if _nav_zone_id_at_success:
                    _current_zone_id = _nav_zone_id_at_success
                ...
                _end_reason = "terminal_explicit_zone_unreachable"
                await _search_not_found(_display, total_checks)
                return
        ...
        # ════════════════════════════════════════════════════════════
        if arrived_zone:
            _local_search_started = True
            logger.info(
                f"[SEARCH sid={_my_sid}] LOCAL_SEARCH handoff STARTED at "
                f"'{_current_zone_id}' — object-search phase active"
            )
            logger.info(
                f"🔒 [SCAN] LOCAL SEARCH MODE: in-room scan starting at "
                f"'{_current_zone_id}' — LiDAR-ranked ≤3 scan directions, no global wander"
            )
            speak_tts_bg("ถึงแล้วครับ กำลังมองรอบๆ")
            ...
            # LiDAR-ranked scan plan: score candidate headings by LiDAR openness.
            # Traversal is greedy nearest-angle: always rotate to the closest
            # remaining heading first, never swing more than _MAX_SINGLE_TURN°.
            _local_angles = _lidar_ranked_scan_headings(_obstacle_avoidance)
            _MAX_SINGLE_TURN = 100.0  # degrees — hard cap per rotation step
            _COVERAGE_HEADINGS_MIN = 6    # unique headings required before NOT_FOUND allowed
            _COVERAGE_DEG_MIN = 300.0     # equiv angular coverage (headings × 45°) before NOT_FOUND
            _SUPP_CANDIDATES = [0.0, 45.0, -45.0, 90.0, -90.0, 135.0, -135.0, 180.0]
            _visited_headings: set = set()
            _scan_facing = 0.0        # current absolute heading relative to room entry
            _scan_remaining = list(_local_angles)
            logger.info(
                f"🏠 room reached → starting in-room search "
                f"(sid={_my_sid} headings_abs={_local_angles} max_turn={_MAX_SINGLE_TURN}°)"
            )
            _scan_step = 0
            while _scan_remaining:
                # Pick the candidate requiring the smallest rotation from current facing
                _target_abs = min(
                    _scan_remaining,
                    key=lambda h: abs((h - _scan_facing + 180) % 360 - 180),
                )
                _la_delta = (_target_abs - _scan_facing + 180) % 360 - 180  # signed
                _scan_remaining.remove(_target_abs)
                if _heading_covered(_target_abs, _visited_headings):
                    logger.info(
                        f"🏠 [SCAN sid={_my_sid}] {_target_abs:+.0f}° covered (±20°) — skipping"
                    )
                    continue
                if abs(_la_delta) > _MAX_SINGLE_TURN:
                    logger.info(
                        f"🏠 [SCAN sid={_my_sid}] {_target_abs:+.0f}° needs "
                        f"{_la_delta:+.0f}° turn > {_MAX_SINGLE_TURN}° cap — stopping scan"
                    )
                    break
                if _scan_step > 0:
                    if _search_cancel or not _session_owner():
                        if _session_owner():
                            ...
                            logger.info(f"🏠 [SCAN sid={_my_sid}] stale — newer session active, exit")
                        return
                    logger.info(f"🏠 [SCAN sid={_my_sid}] heading {_target_abs:+.0f}° (delta {_la_delta:+.0f}°)")
                    await _rotate_deg(_la_delta)
                _scan_facing = _target_abs
                _visited_headings.add(_target_abs)
                _scan_step += 1
                _cov_now = len(_visited_headings)
                logger.info(
                    f"[SCAN_COVERAGE] checked heading={_target_abs:+.0f}° "
                    f"unique_headings={_cov_now} accumulated≈{_cov_now * 45:.0f}°"
                )
                if await _do_vlm_and_check(f"local_zone_{_scan_step}", force=True):
                    return  # found

            # ── Phase B: Supplemental coverage sweep ─────────────────────────
            # Phase A used LiDAR-ranked top-3 (best openness first). If that
            # did not achieve the coverage target, sweep remaining unvisited
            # headings from the full 8-candidate pool. Uses continue (not break)
            # on cap so reachable headings are never skipped due to pool order.
            _phA_count = len(_visited_headings)
            _phA_deg = _phA_count * 45.0
            logger.info(
                f"[SCAN_COVERAGE] Phase A complete: unique_headings={_phA_count} "
                f"accumulated≈{_phA_deg:.0f}° "
                + ("(target met)" if _phA_count >= _COVERAGE_HEADINGS_MIN or _phA_deg >= _COVERAGE_DEG_MIN
                   else "(below target — starting Phase B)")
            )
            if _phA_count < _COVERAGE_HEADINGS_MIN and _phA_deg < _COVERAGE_DEG_MIN:
                _supp_remaining = [
                    h for h in _SUPP_CANDIDATES
                    if not _heading_covered(h, _visited_headings)
                ]
                logger.info(f"[SCAN_COVERAGE] Phase B supplemental pool: {_supp_remaining}")
                while _supp_remaining:
                    _supp_abs = min(
                        _supp_remaining,
                        key=lambda h: abs((h - _scan_facing + 180) % 360 - 180),
                    )
                    _supp_delta = (_supp_abs - _scan_facing + 180) % 360 - 180
                    _supp_remaining.remove(_supp_abs)
                    if _heading_covered(_supp_abs, _visited_headings):
                        continue
                    if abs(_supp_delta) > _MAX_SINGLE_TURN:
                        _cov_now = len(_visited_headings)
                        if _cov_now >= _COVERAGE_HEADINGS_MIN or _cov_now * 45.0 >= _COVERAGE_DEG_MIN:
                            logger.info(
                                f"[SCAN_COVERAGE] Phase B: {_supp_abs:+.0f}° needs "
                                f"{_supp_delta:+.0f}° > cap; coverage met ({_cov_now} hdgs) — stopping"
                            )
                            break
                        logger.info(
                            f"[SCAN_COVERAGE] Phase B: {_supp_abs:+.0f}° needs "
                            f"{_supp_delta:+.0f}° > cap; coverage not met ({_cov_now} hdgs) — skipping"
                        )
                        continue
                    if _search_cancel or not _session_owner():
                        if _session_owner():
                        ...
                        return
                    logger.info(
                        f"[SCAN_COVERAGE] Phase B: rotating to {_supp_abs:+.0f}° (delta {_supp_delta:+.0f}°)"
                    )
                    await _rotate_deg(_supp_delta)
                    _scan_facing = _supp_abs
                    _visited_headings.add(_supp_abs)
                    _scan_step += 1
                    _cov_now = len(_visited_headings)
                    logger.info(
                        f"[SCAN_COVERAGE] checked heading={_supp_abs:+.0f}° "
                        f"unique_headings={_cov_now} accumulated≈{_cov_now * 45:.0f}°"
                    )
                    if await _do_vlm_and_check(f"local_zone_{_scan_step}", force=True):
                        return
                    if _cov_now >= _COVERAGE_HEADINGS_MIN or _cov_now * 45.0 >= _COVERAGE_DEG_MIN:
                        logger.info(
                            f"[SCAN_COVERAGE] coverage target met ({_cov_now} hdgs ≈{_cov_now * 45:.0f}°) "
                            f"→ allowing NOT_FOUND if needed"
                        )
                        break

            # ── In-room expansion: after rotations failed, take ONE cautious
            # forward step (if clearance allows) followed by a fresh scan.
            # Hard-blocked by both wide-cone and direct-forward clearance gates.
            # Stop immediately if the robot drifts out of the target zone.
            ...
            for _ei in range(_MAX_EXPANSION):
                ...
                if _expand_zone_id and _ez_id != _expand_zone_id:
                    ...
                    break
                # Gate 1: wide-cone clearance — blocks marginal/table-edge scenes
                try:
                    _fwd_clear = _obstacle_avoidance.get_forward_clearance()
                except Exception:
                    _fwd_clear = 0.0
                if _fwd_clear < 0.45:
                    logger.info(
                        f"🔒 LOCAL EXPANSION step {_ei+1}: wide_front "
                        f"{_fwd_clear:.2f}m < 0.45m — scanning in place, no nudge"
                    )
                    # Blocked: nearest-angle scan in place (can't move forward anyway).
                    _bl_visited: set = set()
                    _bl_facing = 0.0
                    _bl_remaining = list(_local_angles)
                    _bl_step = 0
                    while _bl_remaining:
                        _bl_abs = min(
                            _bl_remaining,
                            key=lambda h: abs((h - _bl_facing + 180) % 360 - 180),
                        )
                        _bl_delta = (_bl_abs - _bl_facing + 180) % 360 - 180
                        _bl_remaining.remove(_bl_abs)
                        if _heading_covered(_bl_abs, _bl_visited):
                            continue
                        if abs(_bl_delta) > _MAX_SINGLE_TURN:
                            logger.info(
                                f"🔒 [SCAN] blocked {_bl_abs:+.0f}° needs "
                                f"{_bl_delta:+.0f}° > cap — stop"
                            )
                            break
                        if _bl_step > 0:
                            if _search_cancel or not _session_owner():
                                if _session_owner():
                                    await _search_cancelled(_display)
                                return
                            await _rotate_deg(_bl_delta)
                        _bl_facing = _bl_abs
                        _bl_visited.add(_bl_abs)
                        _bl_step += 1
                        if await _do_vlm_and_check(
                            f"local_blocked_{_ei}_{_bl_step}", force=True,
                        ):
                            return
                    break
                # Gate 2: direct narrow-cone — hard-blocks heading straight into obstacle
                try:
                    _dir_fwd, _, _ = _obstacle_avoidance._directional_clearance(0.0, window_deg=7.5)
                except Exception:
                    _dir_fwd = 0.0
                if not math.isfinite(_dir_fwd) or _dir_fwd < 0.40:
                    logger.info(
                        f"🔒 LOCAL EXPANSION step {_ei+1}: "
                        f"direct_fwd={_dir_fwd:.2f}m < 0.40m — blocking nudge"
                    )
                    break
                logger.info(
                    f"🔒 LOCAL EXPANSION step {_ei+1}/{_MAX_EXPANSION}: "
                    f"nudging forward 0.30m inside '{_expand_zone_id}' "
                    f"(wide={_fwd_clear:.2f}m dir={_dir_fwd:.2f}m sid={_my_sid})"
                )
                try:
                    await motion.exec_motion(
                        {"type": "move", "linear_x": 0.15, "angular_z": 0.0,
                ...
                if not _session_owner():
                    logger.info(f"🔒 LOCAL EXPANSION (sid={_my_sid}) stale after nudge — exit")
                    return
                # Fresh LiDAR-ranked plan after nudge (new vantage — reset visited).
                _nd_angles = _lidar_ranked_scan_headings(_obstacle_avoidance)
                _nd_visited: set = set()
                _nd_facing = 0.0
                _nd_remaining = list(_nd_angles)
                _nd_step = 0
                while _nd_remaining:
                    _nd_abs = min(
                        _nd_remaining,
                        key=lambda h: abs((h - _nd_facing + 180) % 360 - 180),
                    )
                    _nd_delta = (_nd_abs - _nd_facing + 180) % 360 - 180
                    _nd_remaining.remove(_nd_abs)
                    if _heading_covered(_nd_abs, _nd_visited):
                        continue
                    if abs(_nd_delta) > _MAX_SINGLE_TURN:
                        logger.info(
                            f"🔒 [SCAN] post-nudge {_nd_abs:+.0f}° needs "
                            f"{_nd_delta:+.0f}° > cap — stop"
                        )
                        break
                    if _nd_step > 0:
                        if _search_cancel or not _session_owner():
                            if _session_owner():
                                await _search_cancelled(_display)
                            return
                        await _rotate_deg(_nd_delta)
                    _nd_facing = _nd_abs
                    _nd_visited.add(_nd_abs)
                    _nd_step += 1
                    if await _do_vlm_and_check(
                        f"local_expand_{_ei}_{_nd_step}", force=True,
                    ):
                        return

            _final_cov = len(_visited_headings)
            logger.info(
                f"[SCAN_COVERAGE] unique_headings={_final_cov} accumulated≈{_final_cov * 45:.0f}° "
                f"→ coverage target met" if _final_cov >= _COVERAGE_HEADINGS_MIN or _final_cov * 45.0 >= _COVERAGE_DEG_MIN
                else f"[SCAN_COVERAGE] unique_headings={_final_cov} accumulated≈{_final_cov * 45:.0f}° "
                f"→ coverage below target (safety/expansion blocked further scan)"
            )
            logger.info("🔒 LOCAL SEARCH MODE: target not found in room — ending search")
            _end_reason = "terminal_local_search_not_found"
            await _search_not_found(_display, total_checks)
            return
        ...
        _end_reason = "terminal_agent_loop_not_found"
        await _search_not_found(_display, total_checks)
        
    except Exception as e:
        logger.error(f"❌ Visual search error: {e}")
        _end_reason = f"terminal_exception:{type(e).__name__}"
        await _broadcast_and_cache({
            "type": "search_status",
            "status": "error",
            "target": _display,
        ...
        if not MOCK_ROBOT:
            try:
                ...
                pass
        # ── Session-owner guard ────────────────────────────────────────
        # Only clear shared search state if THIS task still owns the session.
        # If a newer visual_search has already bumped _search_session_id, its
        # globals (_search_active, _current_search_target, _cancel_checker) are
        # live and must NOT be clobbered by this stale task's finally block.
        global _active_search_task
        if _my_sid == _search_session_id:
            # Sanity: a "transition_*" reason reaching finally means the handoff
            # block ran but LOCAL_SEARCH never actually started — that is the
            # exact success-path handoff bug we are guarding. Log it LOUDLY so
            # it cannot silently hide in normal-looking "state cleared" output.
            if _end_reason.startswith("transition_") and not _local_search_started:
                logger.error(
                    f"[SEARCH sid={_my_sid}] HANDOFF_BUG: terminal cleanup with "
                    f"_end_reason='{_end_reason}' but LOCAL_SEARCH never started — "
                    f"this is the nav-success→terminal bug; investigate the path "
                    f"between arrived_zone=True and LOCAL SEARCH MODE entry"
                )
            _search_active = False
            _search_cancel = False
            _search_cancel_event.clear()
            ...
            _active_search_task = None
            logger.info(
                f"🔍 VISUAL SEARCH END (sid={_my_sid}) — state cleared "
                f"[reason={_end_reason}, local_search_ran={_local_search_started}]"
            )
        else:
            logger.info(
                f"🔍 VISUAL SEARCH END (sid={_my_sid}) STALE — "
                f"newer session {_search_session_id} owns globals; not clearing"
            )
...
async def _search_found(target: str, location: str, description: str):
    ...
    _MAX_FWD_MOVES = 2           # hard cap: max 2 forward motions total
    _fwd_moves_taken = 0
    _consecutive_near_visible = 0
    ...
    for step in range(MAX_APPROACH_STEPS):
        ...
        if not MOCK_ROBOT:
            ...
            if front_clearance < 0.25:
                ...
                break
            # Wide-cone clearance: table-safe stop — catches edge/leg proximity
            _ap_wide = _obstacle_avoidance.get_forward_clearance()
            if _ap_wide < 0.45:
                logger.info(
                    f"   📏 Wide-cone {_ap_wide:.2f}m < 0.45m — "
                    f"object nearby, stopping approach (table-safe)"
                )
                speak_tts_bg(f"ใกล้พอแล้วครับ {target} อยู่ตรงนี้")
                break
        ...
        # Hard cap: 1.0s max per move (table-safe — prevents creeping into legs/edges)
        ...
        # Track forward move count — hard cap for table-safe approach
        _fwd_moves_taken += 1
        if _fwd_moves_taken >= _MAX_FWD_MOVES:
            logger.info(
                f"   🛑 Forward cap reached ({_MAX_FWD_MOVES} moves) — stopping approach"
            )
            break
        ...
        if found:
            ...
            # Consecutive nearby-visible early stop: 2 checks near+confident = done
            _near_locs = {"bottom_left", "bottom_center", "bottom_right",
                          "center_left", "center_right"}
            if new_location in _near_locs and _approach_conf >= 0.75:
                _consecutive_near_visible += 1
                if _consecutive_near_visible >= 2:
                    logger.info(
                        f"   ✅ Visible nearby for {_consecutive_near_visible} checks "
                        f"(conf={_approach_conf:.2f}) — close enough, stopping"
                    )
                    break
            else:
                _consecutive_near_visible = 0
            ...
            if (is_close and is_center) or (step >= 2 and _approach_conf >= 0.8 and is_center) or step >= 3:
                reason_text = "center/near" if (is_close and is_center) else f"conf={_approach_conf:.1f},step={step+1}" if step >= 2 else "max_steps"
...
async def execute_server_command(cmd: str, params: dict):
    ...
    try:
        if cmd == "move":
            ...
            if motion_cmd:
                speak_tts_bg(f"{'เดินหน้า' if direction == 'forward' else 'ถอยหลัง'}ครับ")
                if not MOCK_ROBOT:
            ...
            direction_word = "ซ้าย" if angle > 0 else "ขวา"
            speak_tts_bg(f"หมุน{direction_word}ครับ")
            if not MOCK_ROBOT:
                await motion.exec_motion(motion_cmd)
            ...
            if target:
                ...
                await _start_visual_search_task(lambda: visual_search(target))
            else:
...
_DANGER_CONSECUTIVE_REQUIRED = 4  # 4 × 0.2s = 0.8s sustained below threshold to cancel
                                   # Nav2 rotation spikes are 1–2 samples; true collision is sustained
...
async def _nav_safety_watchdog(cancel_event: asyncio.Event) -> Dict[str, Any]:
    ...
    _danger_count = 0  # consecutive samples below _SAFE_FORWARD_DANGER
    while not cancel_event.is_set():
        # (A) Hard collision guard — sustained imminent contact only.
        # Single-sample spikes during Nav2 rotation (robot briefly faces a wall in the
        # ±16–60° cone while turning) must NOT cancel a valid nav goal.
        # Require _DANGER_CONSECUTIVE_REQUIRED consecutive readings (0.8s) before cancel.
        try:
        ...
        if fwd < _SAFE_FORWARD_DANGER:
            _danger_count += 1
            logger.warning(
                f"[SAFETY] fwd={fwd:.2f}m < {_SAFE_FORWARD_DANGER} "
                f"({_danger_count}/{_DANGER_CONSECUTIVE_REQUIRED}) — rotation or sustained wall?"
            )
            if _danger_count >= _DANGER_CONSECUTIVE_REQUIRED:
                logger.warning(
                    f"[SAFETY] emergency stop — {_danger_count} sustained readings ≤ {fwd:.2f}m — cancel Nav2"
                )
                try:
...
async def gw_audio(ws: WebSocket):
    ...
    async def on_server_text(msg: str):
        ...
        if kind in ["final","result","transcript_final"] and text:
            ...
            # --- Thai command normalization (conservative) ---
            _norm = normalize_thai_command(_stripped)
            logger.info(f"🧹 NORMALIZED: '{_norm}'")

            # ================================================================
            # PRIORITY 1: Deterministic control intents (bypass wakeword + LLM)
            # cancel / repeat / status / where_am_i / return_home / stop
            # ================================================================
            _ctrl = parse_control_intent(_norm)
            if _ctrl:
                logger.info(f"🎯 CONTROL INTENT: {_ctrl['intent']}")
                await handle_control_intent(_ctrl["intent"], _stripped, _norm)
                logger.info(f"✅ Control done in {(time.monotonic()-_t0_cmd)*1000:.0f}ms")
                logger.info(f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
                return
            ...
            if multi_objects:
                ...
                # Search for multiple objects sequentially (cancel + session-guarded relaunch)
                await _start_visual_search_task(lambda: visual_search_multi(multi_objects))
                return
            ...
            if desc_query:
                ...
                if _search_active and _current_search_target == target_with_desc:
                    logger.info(f"[DEDUP] same search '{target_with_desc}' already active — ignoring duplicate")
                    return

                # Use full description in VLM query (cancel + session-guarded relaunch)
                await _start_visual_search_task(lambda: visual_search(target_with_desc))
                return
            ...
            if find_target:
                ...
                if _search_active and _current_search_target == find_target:
                ...
                await _start_visual_search_task(lambda: _enrich_then_search_audio())
                return
            ...
            # 3. Task-oriented fallback — never leak into open-domain chat.
            _post_norm = normalize_thai_command(text)
            await _task_oriented_fallback(
                raw=text, normalized=_post_norm,
                command_like=looks_like_command(_post_norm),
            )
...
async def gw_text(inp: TextIn):
    ...
    # 0. Normalize + priority control-intent router (bypass LLM)
    _norm = normalize_thai_command(text)
    _ctrl = parse_control_intent(_norm)
    if _ctrl:
        await handle_control_intent(_ctrl["intent"], text, _norm)
        return {"ok": True, "mode": "control", "intent": _ctrl["intent"], "normalized": _norm}

    # 1. ลอง Motion Parser ก่อน (Regex → /cmd_vel ตรง, เร็ว)
    motion_cmd = parse_intent(text) or parse_intent(_norm)
    if motion_cmd:
    ...
    if find_target:
        ...
        await _start_visual_search_task(lambda: _enrich_then_search_text())
    ...
    # 3. Task-oriented fallback. Normal runtime mode does NOT answer open-domain
    #    chat. Command-shaped inputs → clarify. Anything else → out-of-scope.
    command_like = looks_like_command(_norm)
    await _task_oriented_fallback(raw=text, normalized=_norm, command_like=command_like)
    return {
        "ok": True,
        "mode": "clarify" if command_like else "out_of_scope",
        "reply": CLARIFY_TH if command_like else OUT_OF_SCOPE_TH,
        "normalized": _norm,
    }