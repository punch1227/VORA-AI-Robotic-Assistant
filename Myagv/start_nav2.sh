...
if [[ "$MODE" == "nav" ]]; then
    ...
    # wait_amcl_active polls /amcl/get_state directly (up to 40s) — the only reliable
    # gate before publishing /initialpose. A fixed sleep races with Jetson Nano's slow
    # DDS discovery; if AMCL is not yet active, /initialpose is silently dropped and
    # map→odom TF is never emitted, so the robot navigates from wrong initial pose.
    wait_amcl_active || exit 1

    # ── TF freshness guard — the real fix for "extrapolation into the future" ──
    # AMCL's handleInitialPoseMessage runs a DUAL-TIME TF lookup:
    #   tf_buffer->lookupTransform(base_frame, msg.header.stamp,
    #                              base_frame, now(),
    #                              odom_frame, 1.0 s)
    # Backdating msg.header.stamp only helps the first half of that lookup.  The
    # SECOND half (at AMCL's own now()) still throws "extrapolation into the
    # future" whenever AMCL's /tf buffer holds no entry newer than ~1 s — which
    # happens on Jetson Nano right after AMCL activates but before its DDS /tf
    # subscription has matched with odom_tf_broadcaster.py.
    #
    # Probe from THIS script's own tf buffer; if this process sees an
    # odom→base_footprint entry younger than 300 ms, AMCL on the same DDS domain
    # has one too, so both halves of its dual-time lookup will succeed.
    info "Verifying odom→base_footprint TF freshness before /initialpose (up to 20s)..."
    if ! python3 - <<'PY'
import time
import rclpy
from rclpy.node import Node
...
rclpy.init()
node = Node("vora_wait_odom_fresh")
buf = Buffer()
listener = TransformListener(buf, node, spin_thread=True)
...
while time.time() < deadline:
    try:
        rclpy.spin_once(node, timeout_sec=0.1)
        if buf.can_transform("odom", "base_footprint", rclpy.time.Time(),
                             timeout=Duration(seconds=0.2)):
            tf = buf.lookup_transform("odom", "base_footprint", rclpy.time.Time())
...
PY
    then
        err "odom→base_footprint TF not fresh after 20s — DDS is not delivering TFs"
        err "  Debug: ros2 run tf2_ros tf2_echo odom base_footprint"
        exit 1
    fi
    ok "odom→base_footprint TF is fresh (<300ms old) — proceeding with /initialpose"

    # Publish initial pose via a rclpy node.  Stamp is backdated 300 ms so AMCL's
    # first-half TF lookup (at msg.header.stamp) lands well inside the buffer;
    # the freshness guard above handles AMCL's second-half lookup at now().
    info "Publishing initial pose 3× (${INIT_X}, ${INIT_Y}, yaw=${INIT_YAW}) stamp=now−0.3s..."
    python3 - <<PYEOF
import rclpy
from rclpy.node import Node
from rclpy.duration import Duration
...
deadline_sub = time.time() + 15.0
while time.time() < deadline_sub:
    if pub.get_subscription_count() > 0:
        break
    rclpy.spin_once(node, timeout_sec=0.1)
if pub.get_subscription_count() == 0:
    import sys
    node.get_logger().error('AMCL /initialpose subscriber not discovered after 15s — is AMCL active?')
    node.destroy_node()
    rclpy.shutdown()
    sys.exit(1)
...
for i in range(3):
    ...
    # Stamp 0.3 s in the past so AMCL's first-half lookup (at msg.header.stamp)
    # lands inside the existing buffer, not ahead of the newest entry.
    stamp_time = node.get_clock().now() - Duration(nanoseconds=300_000_000)
    ...
    node.get_logger().info(
        f'Initial pose {i+1}/3 — stamp={msg.header.stamp.sec}.{msg.header.stamp.nanosec:09d} (now-0.3s)'
    )
    time.sleep(1.0)
...
PYEOF
    ...
    warn "[NAV2] Skipping wait_nav2_active for MVP on Jetson/Galactic"
