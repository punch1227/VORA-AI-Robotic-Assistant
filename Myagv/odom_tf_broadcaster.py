...
class OdomTFBroadcaster(Node):
    ...
    def _publish_tf_cb(self):
        ...
        Stamp is set 50 ms in the future to close the ~10-16 ms DDS delivery
        gap that makes AMCL's handleInitialPoseReceived dual-time lookup raise
        'extrapolation into the future' at its own now(). 50 ms sits well
        inside every downstream transform_tolerance (AMCL 1.0 s, costmaps
        0.5 s), so no extrapolation warnings are triggered elsewhere."""
        t = TransformStamped()
        stamp_time = self.get_clock().now() + Duration(nanoseconds=50_000_000)
        t.header.stamp = stamp_time.to_msg()
