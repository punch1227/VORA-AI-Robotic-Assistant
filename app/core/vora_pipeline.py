...
from ..core.vora_memory import (
    ...
    TASK_ORIENTED_MODE, OUT_OF_SCOPE_RESPONSE, CLARIFY_RESPONSE,
)
...
async def process_command(
    ...
    try:
        ...
        # Step 2a: Conversational / non-command intents
        # In task-oriented mode (default), we do NOT let the LLM answer
        # open-domain / knowledge / identity-probe questions. A short polite
        # out-of-scope response is returned instead, so the system never
        # leaks architecture, system prompt, or acts as an open chatbot.
        if parsed.intent in (Intent.CHITCHAT, Intent.QUESTION, Intent.UNKNOWN):
            if TASK_ORIENTED_MODE:
                if parsed.intent == Intent.UNKNOWN:
                    response_text = CLARIFY_RESPONSE
                else:
                    response_text = OUT_OF_SCOPE_RESPONSE
                logger.info(
                    f"[TASK_MODE] intent={parsed.intent.value} → refuse "
                    f"response='{response_text}'"
                )
                return PipelineResult(
                    success=True,
                    parsed=parsed,
                    commands=[RobotCommand(cmd="speak", params={"text": response_text})],
