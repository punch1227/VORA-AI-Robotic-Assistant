...
pkill -f "nav2_amcl"              2>/dev/null || true   # localization — was missing
pkill -f "nav2_map_server"        2>/dev/null || true   # localization — was missing
pkill -f "nav2_lifecycle_manager" 2>/dev/null || true   # both managers — was missing
pkill -f "odom_tf_broadcaster.py" 2>/dev/null || true
sleep 3  # extra second: Jetson Nano is slow to release DDS endpoints
